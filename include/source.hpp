...
struct NextCharacter
{
    ...
    std::string getLinePosition() const { return std::to_string(characterPosition) + ":" + std::to_string(linePosition); }
...
public:
    ...
    FileSource(const std::string_view filepath) : filepath(std::filesystem::path(filepath)),
                                                   position(Position()){}
    ~FileSource(){
        close();
    }
